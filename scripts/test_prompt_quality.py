...
for name, msg in test_cases:
    ...
    for e in events:
        print(
            f"  action={e.get('action')} object={e.get('object_name_raw')[:60]} status={e.get('status')}"
        )
        if e.get("summary"):
...
for (name, is_event, events), exp in zip(results, expected, strict=False):
    ok = not is_event if exp is False else is_event and len(events) <= exp
